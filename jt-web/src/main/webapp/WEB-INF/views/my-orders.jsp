    ...
    <tbody id="tb-2538292730">
        ...
        <tr id="track2538292730" oty="0,1,70" class="tr-td">
            <td>
            	<div class="img-list">
					<c:forEach items="${order.orderItems}" var="item" varStatus="status">
            		        		  		<a href="http://www.jt.com/items/${item.itemId}.html" class="img-box" clstag="click|keycount|orderinfo|order_product" target="_blank">
        	...
        	<td>
                ...
                <c:if test="${order.paymentType == 2}">货到付款</c:if><br>
        	            	</td>
            <td>
                <span class="ftx-03">
				<!--设置时间格式  -->
				<fmt:formatDate value="${order.created}" pattern="yyyy-MM-dd HH:mm:ss" /></span>
                <input type="hidden" id="datasubmit-2538292730" value="${order.created}">
            </td>

            <td><span class="ftx-03"><c:if test="${order.status == 1}">未付款</c:if>
